...
export enum RealTimeMode {
    NONE = 'none',
    POLLING_REPLACE = 'polling_replace',
    POLLING_APPEND = 'polling_append',
}
...
export interface IRealTimeConfig {
    ...
    intervalMs: number;
    /** For POLLING_APPEND: field ID used as cursor (e.g. a timestamp or auto-increment column) */
    cursorField?: string;
    /** Last seen cursor value for incremental fetches */
    lastCursor?: any;
    /** Ring-buffer limit: trim oldest rows once dataset exceeds this size */
    maxRows?: number;
    /** Debounce (ms) before downstream analysis re-runs after a data update */
    analysisDebounceMs: number;
    autoRetrigger: {
        semiAuto: boolean;
        causal: boolean;
    };
    /** Max retry attempts per poll before signalling error (default 3) */
    maxRetries: number;
}

export const DEFAULT_REALTIME_CONFIG: IRealTimeConfig = {
    mode: RealTimeMode.NONE,
    intervalMs: 30_000,
    analysisDebounceMs: 5_000,
    autoRetrigger: { semiAuto: false, causal: false },
    maxRetries: 3,
};

export interface IDataFetchRecipe {
    type: 'database' | 'restful';
    // Database
    dbServer?: string;
    dbSourceType?: string;
    dbUri?: string;

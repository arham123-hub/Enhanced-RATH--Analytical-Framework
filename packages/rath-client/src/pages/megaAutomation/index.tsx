        ...
        <div className="content-container">
            ...
            <Card>
                ...
                <Toggle
                    inlineLabel
                    label="Progressive"
                    title="Run insight discovery on a 5% sample for instant results, then refine on 25% and 100%."
                    checked={ltsPipeLineStore.progressiveMode}
                    onChange={(_e, checked) => ltsPipeLineStore.setProgressiveMode(Boolean(checked))}
                    styles={{ root: { float: 'right', marginRight: '1em', marginBottom: 0 } }}
                />
                <ComputationProgress />
                {ltsPipeLineStore.progressivePhase && (
                    <div
                        style={{
                            ...
                            marginLeft: '1em',
                            borderRadius: 4,
                            background: ltsPipeLineStore.progressivePhase.phase === 'full' ? '#e8f5e8' : '#e8f1fb',
                            color: ltsPipeLineStore.progressivePhase.phase === 'full' ? '#107c10' : '#0078d4',
                            fontSize: 12,
                            fontWeight: 600,
                        }}
                    >
                        {ltsPipeLineStore.progressivePhase.phase === 'full' ? 'Final' : 'Approx.'}{' '}
                        {ltsPipeLineStore.progressivePhase.label} ·{' '}
                        {ltsPipeLineStore.progressivePhase.sampleSize.toLocaleString()} /{' '}
                        {ltsPipeLineStore.progressivePhase.populationSize.toLocaleString()} rows
                    </div>
                )}

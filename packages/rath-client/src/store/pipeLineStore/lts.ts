...
export class LTSPipeLine {
    ...
    public progressiveMode: boolean = false;
    public progressivePhase: { phase: ProgressivePhase; label: string; sampleSize: number; populationSize: number } | null = null;
    ...
    public setProgressiveMode (enabled: boolean) {
        this.progressiveMode = enabled;
        if (!enabled) this.progressivePhase = null;
    }
    private async runEngineStart (
        taskMode: ITaskTestMode,
        dataSource: IRow[],
        fieldMetas: DataSourceStore['fieldMetas'],
        limit: PreferencePanelConfig['viewSizeLimit'],
    ) {
        if (taskMode === ITaskTestMode.local) {
            return rathEngineService({
                task: 'start',
                props: {
        ...
        return rathEngineServerService({
            task: 'start',
            dataSource,
            fields: fieldMetas,
            props: {
    ...
    public async startTask (taskMode: ITaskTestMode = ITaskTestMode.local, limit: PreferencePanelConfig['viewSizeLimit']) {
        ...
        this.computing = true;
        try {
            const useProgressive = this.progressiveMode && cleanedData.length >= PROGRESSIVE_MIN_ROWS;
            const schedule = useProgressive
                ? buildProgressiveSchedule(cleanedData.length)
                : [{ phase: 'full' as ProgressivePhase, sampleSize: cleanedData.length, indices: [] }];
            for (const slice of schedule) {
                const slicedData = slice.phase === 'full'
                    ? cleanedData
                    : materializeSample(cleanedData, slice.indices);
                runInAction(() => {
                    this.progressivePhase = useProgressive ? {
                        phase: slice.phase,
                        label: phaseLabel(slice.phase),
                        sampleSize: slice.sampleSize,
                        populationSize: cleanedData.length,
                    } : null;
                });
                // eslint-disable-next-line no-await-in-loop
                const phaseRes = await this.runEngineStart(taskMode, slicedData, fieldMetas, limit);
                // eslint-disable-next-line no-console
                PRINT_PERFORMANCE && console.log(phaseRes.performance);
                runInAction(() => {
                    this.insightSpaces = phaseRes.insightSpaces;
                    this.dataSource = phaseRes.dataSource;
                    this.fields = phaseRes.fields;
                });
            }
            runInAction(() => {

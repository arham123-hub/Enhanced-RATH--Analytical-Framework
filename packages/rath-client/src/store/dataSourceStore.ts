...
import { cleanDataService, filterDataService,  inferMetaService, computeFieldMetaService, computeFieldMetaProgressive } from "../services/index";
import type { ProgressivePhase } from "../utils/progressiveSampler";
import { appendSnapshot, ChangeDetectionResult, detectSignificantChange, MetricSnapshot } from "../utils/changeDetection";
...
export class DataSourceStore {
    ...
    public realTimeConfig: IRealTimeConfig = { ...DEFAULT_REALTIME_CONFIG };
    public realTimeStatus: RealTimeStatus = RealTimeStatus.IDLE;
    public lastRefreshTimestamp: number | null = null;
    public refreshCount: number = 0;
    public realTimeError: string | null = null;
    public retryInfo: { retryCount: number; maxRetries: number } | null = null;
    public newRowCount: number = 0;
    private newRowTimer: ReturnType<typeof setTimeout> | null = null;
    // ── Progressive Analytics Engine ────────────────────────────────────
    public progressiveStats: {
    ...
    public progressiveEnabled: boolean = true;
    /**
     * DuckDB-WASM handle when the dataset was ingested via Big File Mode.
     * Null otherwise. Kept on the store so power users can run push-down SQL
     * (`SELECT … FROM rath_big USING SAMPLE 5 PERCENT`) against the full
     * source without re-loading.
     */
    public bigFileHandle: import('@duckdb/duckdb-wasm').AsyncDuckDB | null = null;
    public bigFileTotalRows: number = 0;
    /** Per-field rolling snapshot history for significant-change detection. */
    public metricHistory: Map<string, MetricSnapshot[]> = new Map();
    /** Latest detected significant change per field; clears after expiry. */
    public significantChanges: Map<string, ChangeDetectionResult> = new Map();
    private changeExpiryTimers: Map<string, ReturnType<typeof setTimeout>> = new Map();
    // ───────────────────────────────────────────────────────────────────
    public dataFetchRecipe: IDataFetchRecipe | null = null;
    private pollingSubscription: Subscription | null = null;
    private pollingManager: import('../services/realtime').PollingManager | null = null;
    ...
    constructor() {
        makeAutoObservable(this, {
            ...
            newRowTimer: false,
            metricHistory: observable.shallow,
            significantChanges: observable.shallow,
            changeExpiryTimers: false,
            bigFileHandle: observable.ref,
        });
        this.initStore();
    }
    public clear() {
    ...
    public initStore () {
        ...
        this.datasetId = null;
        // Reset real-time state (but preserve config so user settings survive a re-load)
        this.stopRealTime();
        this.lastRefreshTimestamp = null;
        this.resetMetricHistory();
        this.refreshCount = 0;
        this.realTimeError = null;
        this.dataFetchRecipe = null;
        ...
        // Use analysisDebounceMs from config when polling is active, otherwise 500ms
        const rawDataMetaInfo$ = from(toStream(() => this.rawDataMetaInfo, false)).pipe(
            op.debounce(() => timer(
                this.realTimeStatus === RealTimeStatus.RUNNING
                    ? this.realTimeConfig.analysisDebounceMs
                    : 500
            ))
        );
        ...
        const originFieldMetas$ = cleanedData$.pipe(
            ...
            op.map(([dataSource, fields]) => {
                if (!this.progressiveEnabled) {
                    runInAction(() => {
                        this.progressiveStats = null;
                    });
                    return from(
                        computeFieldMetaService({ dataSource, fields: fields.map(f => toJS(f)) }).then(metas => {
                            runInAction(() => {
                                this.recordMetricsAndDetectChanges(metas);
                            });
                            return metas;
                        }),
                    );
                }
                return new Observable<IFieldMeta[]>(subscriber => {
                    let cancelled = false;
                    computeFieldMetaProgressive(
                        { dataSource, fields: fields.map(f => toJS(f)) },
                        phase => {
                            if (cancelled) return;
                            runInAction(() => {
                                this.progressiveStats = {
                                    phase: phase.phase,
                                    label: phase.label,
                                ...
                                if (phase.phase === 'full') {
                                    this.recordMetricsAndDetectChanges(phase.metas);
                                }
                            });
                            subscriber.next(phase.metas);
                        },
                    ).then(() => {
    ...
    public setProgressiveEnabled (enabled: boolean) {
        this.progressiveEnabled = enabled;
        if (!enabled) this.progressiveStats = null;
    }

    public setBigFileHandle (
        handle: import('@duckdb/duckdb-wasm').AsyncDuckDB | null,
        totalRows: number = 0,
    ) {
        this.bigFileHandle = handle;
        this.bigFileTotalRows = totalRows;
        if (typeof window !== 'undefined') {
            // Expose for demo-time console access. Read-only for users.
            (window as unknown as { __rathBigFile?: unknown }).__rathBigFile = handle
                ? { db: handle, totalRows, table: 'rath_big' }
                : undefined;
        }
    }

    /** Reset all change-detection state. Called on dataset reload. */
    public resetMetricHistory () {
        this.metricHistory = new Map();
        this.significantChanges = new Map();
        this.changeExpiryTimers.forEach(t => clearTimeout(t));
        this.changeExpiryTimers = new Map();
    }

    public dismissSignificantChange (fid: string) {
        this.significantChanges.delete(fid);
        const timer = this.changeExpiryTimers.get(fid);
        if (timer) {
            clearTimeout(timer);
            this.changeExpiryTimers.delete(fid);
        }
        // Trigger MobX update on the shallow Map.
        this.significantChanges = new Map(this.significantChanges);
    }

    /**
     * Record metric snapshots from the latest 100% phase metas and run
     * significant-change detection. Only fires when the user is in real-time
     * mode (so we're comparing across polls, not across a single load).
     */
    private recordMetricsAndDetectChanges (metas: IFieldMeta[]) {
        if (this.realTimeStatus !== RealTimeStatus.RUNNING) return;
        const now = Date.now();
        const newHistory = new Map(this.metricHistory);
        const newChanges = new Map(this.significantChanges);
        for (const m of metas) {
            if (m.semanticType !== 'quantitative') continue;
            const stdev = Number(m.features.stdev);
            const mean = Number(m.features.mean);
            const n = Number(m.features.count);
            if (!Number.isFinite(stdev) || !Number.isFinite(mean) || !(n > 1)) continue;
            const snap: MetricSnapshot = { fid: m.fid, ts: now, mean, stdev, n };
            const prev = newHistory.get(m.fid) ?? [];
            const result = detectSignificantChange(prev, snap, 5);
            newHistory.set(m.fid, appendSnapshot(prev, snap));
            if (result && result.changed) {
                const isNew = !this.significantChanges.has(m.fid);
                newChanges.set(m.fid, result);
                const existing = this.changeExpiryTimers.get(m.fid);
                if (existing) clearTimeout(existing);
                this.changeExpiryTimers.set(
                    m.fid,
                    setTimeout(() => {
                        runInAction(() => {
                            this.significantChanges.delete(m.fid);
                            this.significantChanges = new Map(this.significantChanges);
                            this.changeExpiryTimers.delete(m.fid);
                        });
                    }, 30_000),
                );
                if (isNew) {
                    notify({
                        ...
                        content: `Mean of "${m.name}" shifted by ${result.deltaSigmas.toFixed(1)}σ over the last ${result.gapSeconds.toFixed(0)}s (${result.historicalMean.toFixed(2)} → ${result.currentMean.toFixed(2)}).`,
                    });
                }
            }
        }
        this.metricHistory = newHistory;
        this.significantChanges = newChanges;
    }
    ...
    public async loadDataWithInferMetas (dataSource: IRow[], fields: IMuteFieldBase[], tag?: DataSourceTag | undefined, recipe?: IDataFetchRecipe) {
        if (fields.length > 0 && dataSource.length > 0) {
            ...
            runInAction(() => {
                ...
                // Store the recipe so real-time polling can re-fetch the same data
                this.dataFetchRecipe = recipe ?? null;
                this.updateDataMetaInIndexedDB()
            })
        }
    }
    ...
    public setRealTimeConfig(config: Partial<IRealTimeConfig>) {
        runInAction(() => {
            this.realTimeConfig = { ...this.realTimeConfig, ...config };
        });
    }

    private flashNewRowCount(count: number) {
        this.newRowCount = count;
        if (this.newRowTimer) clearTimeout(this.newRowTimer);
        this.newRowTimer = setTimeout(() => {
            runInAction(() => { this.newRowCount = 0; });
        }, 3_000);
    }

    /**
     * Append new rows to the existing dataset.
     * Triggers the RxJS pipeline automatically via rawDataMetaInfo change.
     */
    public async appendData(newRows: IRow[]) {
        if (newRows.length === 0) return;
        try {
            const existing = await this.rawDataStorage.getAll() ?? [];
            let combined = [...existing, ...newRows];
            const maxRows = this.realTimeConfig.maxRows;
            if (maxRows && combined.length > maxRows) {
                combined = combined.slice(combined.length - maxRows);
            }
            await this.rawDataStorage.setAll(combined);
            runInAction(() => {
                this.rawDataMetaInfo = this.rawDataStorage.metaInfo;
                this.lastRefreshTimestamp = Date.now();
                this.refreshCount++;
                this.flashNewRowCount(newRows.length);
            });
        } catch (err) {
            runInAction(() => {
                this.realTimeError = String(err);
                this.realTimeStatus = RealTimeStatus.ERROR;
            });
        }
    }

    /**
     * Replace the entire dataset with new rows.
     * Triggers the RxJS pipeline automatically via rawDataMetaInfo change.
     */
    public async replaceData(rows: IRow[]) {
        if (rows.length === 0) return;
        try {
            await this.rawDataStorage.setAll(rows);
            runInAction(() => {
                this.rawDataMetaInfo = this.rawDataStorage.metaInfo;
                this.lastRefreshTimestamp = Date.now();
                this.refreshCount++;
                this.flashNewRowCount(rows.length);
            });
        } catch (err) {
            runInAction(() => {
                this.realTimeError = String(err);
    ...
    public startRealTime() {
        if (!this.dataFetchRecipe) {
            notify({ title: 'Real-Time Error', type: 'error', content: 'No data fetch recipe available. Load data from a Database or API source first.' });
            return;
        }

        // If paused, resume the existing manager rather than creating a new one
        if (this.realTimeStatus === RealTimeStatus.PAUSED && this.pollingManager) {
        ...
        // Import is deferred to avoid circular deps; PollingManager is created inline
        import('../services/realtime').then(({ createPollingManager }) => {
            this.pollingSubscription?.unsubscribe();
            this.errorSubscription?.unsubscribe();
            this.pollingManager?.destroy();
            const manager = createPollingManager(this.dataFetchRecipe!, this.realTimeConfig);
            this.pollingManager = manager;

            // Subscribe to error signals for retry feedback
            this.errorSubscription = manager.errors$.subscribe((pollingError) => {
                runInAction(() => {
                    this.retryInfo = {
                        retryCount: pollingError.retryCount,
                        maxRetries: pollingError.maxRetries,
                    };
                    if (!pollingError.willRetry) {
                        this.realTimeError = `Connection failed after ${pollingError.maxRetries} retries: ${pollingError.error.message ?? pollingError.error}`;
                    }
                });
            });

            this.pollingSubscription = manager.data$.subscribe({
                next: (rows) => {
                    runInAction(() => {
                        this.retryInfo = null;
                        this.realTimeError = null;
                    });
                    if (this.realTimeConfig.mode === RealTimeMode.POLLING_APPEND) {
    ...
    public pauseRealTime() {
        this.pollingManager?.pause();
        runInAction(() => {
            this.realTimeStatus = RealTimeStatus.PAUSED;
        });
    }
